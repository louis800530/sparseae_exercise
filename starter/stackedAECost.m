...
eout=groundTruth-bsxfun(@rdivide,output,sum(output));

softmaxThetaGrad=-eout*A3'/M+lambda*softmaxTheta;

delta3=-softmaxTheta'*eout.*A3.*(1-A3);
...
cost=-sum(sum(groundTruth.*log(bsxfun(@rdivide,output,sum(output)))))/M+lambda*0.5*sum(sum(softmaxTheta.^2));
